...
struct Problem : ProblemBase<_GraphT, _FLAG> {
  ...
  */
  struct DataSlice : BaseDataSlice {
    ...
    util::Array1D<SizeT, int> height;              // vertex height
    ...
    util::Array1D<SizeT, VertexT> height_indirection; 
    util::Array1D<SizeT, VertexT> v_que;
    util::Array1D<SizeT, VertexT> d_parent;         
    ...
    util::Array1D<SizeT, VertexT> mark;
    ...
    */
    DataSlice() : BaseDataSlice() {
      ...
      height_indirection.SetName("height_indirection");
      v_que.SetName("v_que");
      //new_residuals.SetName("new_residuals");
    ...
    */
    cudaError_t Release(util::Location target = util::LOCATION_ALL) {
      ...
      GUARD_CU(height_indirection.Release(target));	// added this new line ...
      GUARD_CU(v_que.Release(target));	// added this new line ...
      //GUARD_CU(new_residuals.Release(target));
    ...
    */
    cudaError_t Init(GraphT &sub_graph, int num_gpus = 1, int gpu_idx = 0,
      ...
      GUARD_CU(height_indirection.Allocate(nodes_size, util::HOST|target));    // added this new line...
      GUARD_CU(v_que.Allocate(nodes_size, target));    // added this new line...

      //GUARD_CU(new_residuals.Allocate(edges_size, target));
  ...
  GUARD_CU(height_indirection.EnsureSize_(nodes_size, target));         // added the new line...
  GUARD_CU(v_que.EnsureSize_(nodes_size, target));         // added the new line...
  //GUARD_CU(new_residuals.EnsureSize_(edges_size, target));
...
std::cout << std::endl;
      ...
      GUARD_CU(height.ForAll(
          [source, sink, nodes_size] __host__ __device__(int * h,
                                                        const VertexT &pos) {
            if (pos == source)
              h[pos] = nodes_size;
            else  // if (pos == sink)
          ...
          GUARD_CU(height.ForAll(
            [nodes_size,sink,source] __host__ __device__(int * h, const VertexT &pos) {
              if (pos == sink) h[pos] = -nodes_size;
              else if (pos == source) h[pos] = nodes_size;
              else h[pos] = 0;
            },
            nodes_size, util::DEVICE, this->stream));
  ...
  GUARD_CU(ctr.ForAll(
      ...
      /*GUARD_CU(height_indirection.ForAll(
        [] __host__ __device__(VertexT * height_indirection_, const VertexT &pos) {
          height_indirection_[pos] = (VertexT)pos;
        },
        nodes_size, target, this->stream));*/
      GUARD_CU(v_que.ForAll(
          [] __host__ __device__(VertexT * v_que_, const VertexT &pos) {
            v_que_[pos] = 0;
          },
          nodes_size, target, this->stream));
