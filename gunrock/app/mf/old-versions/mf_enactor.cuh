   ...
   auto &numofrelabel = data_slice.numofrelabel;		// added the new line...
    ...
    auto compute_lockfree_op =
        [graph, excess, residuals, reverse, height, iteration, source, sink,
         active, numofrelabel, lowest_neighbor, ifexcess, ctr] __host__
	...
	if(EXCESS_CYCLE_REMOVE)
	{
        	if(v == 0){                  // added
			 ...
			 ctr[0] = ctr[0]+1;
       		...
       		if( ifexcess[counter%2] == 0 )
			return;
		
	}

	  if( graph.CsrT::GetNeighborListLength(v) == 0  ) return;
	  ...
	  if(v == 0) numofrelabel[(counter + 1) % 2] = 0;   
          ...
          if (excess_v < ( THRESHOLD == 0 ? MF_EPSILON : THRESHOLD  ) || neighbor_num == 0) return;     // Added logic for memory access pruning
