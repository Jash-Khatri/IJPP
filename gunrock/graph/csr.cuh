...
//std::cout << source.CooT::directed << " \n";
if(ENABLE_VERTEX_RENUMBERING && UNDIRECTED )
{
	std::cout << "Performing the Vertex Renumbering\n";
	        // Get starting timepoint
        auto start = high_resolution_clock::now();

        // sorting the nodes according to degrees

        std::vector <VertexT> degree(nodes,0);
        std::vector <VertexT> sortedindices(nodes);
        auto max_degree=0;
        for (auto u = 1; u < nodes-2; ++u)
        {
                degree[u] =  GetNeighborListLength(u);
	...
	sortedindices[index++] = nodes-2;
        sortedindices[index++] = nodes-1;
        ...
        std::vector <VertexT> OA(nodes+1);
        std::vector <VertexT> CA(edges);                  // its size is equal to edges
        std::vector <VertexT> reversesortedindices(nodes);
	
	std::vector <ValueT> sortededges(edges,1);	
		...
		if( sortedindices[i] == 0 )
			std::cout << i << "\n";
		if( sortedindices[i] == nodes-1 )
                        std::cout << i << "\n";
		if( sortedindices[i] == nodes-2 )
                        std::cout << i << "\n";

                auto e_start = GetNeighborListOffset(x);
                auto num_neighbors = GetNeighborListLength(x);
                auto e_end = e_start + num_neighbors;

                for (auto e = e_start; e < e_end; ++e){
                        CA[index] = reversesortedindices[GetEdgeDest(e)] ;
			//if (FLAG & HAS_EDGE_VALUES)
				sortededges[index] = edge_values[e]; 		
                        //std::cout << CA[index] << " ";
                        index++;
                }
	...
	/*	
	// Printing for the checking purposes
	...
	*/
	...
	// assign edge_values
       ...
       // No need to move the OA and CA now to device
        ...
        std::cout << "Time taken by vertex renumbering function: " << duration.count() << " microseconds" << std::endl;
}
