...
bool comparetwop1s(pairs p1, pairs p2){
...
bool quiet = parameters.Get<bool>("quiet");
  ...
  for (int i = 0; i < edges; ++i) {
    ...
    //adding the edge weights to the adjmat
...
auto start1 = high_resolution_clock::now();
    //calculate the out-degree of all the vertices this will be useful for finding right structure    
    int *degree = (int *)malloc( (nodes + 1)*sizeof(int) );
...
double Y; // An constant
Y = SPECT_SPARSE_P*( (double)edges/nodes);
double edge_stay; 
long long int edge_removed_cnt = 0;

for(int i=0;i<edges;i++){  // remove the edges using spectral sparsification.

	if( ( std::min( degree[edge_pairs[i].x] , degree[edge_pairs[i].y] ) ) > 0 ){

	edge_stay = std::min( 1.0 , Y / ( std::min( degree[edge_pairs[i].x] , degree[edge_pairs[i].y] ) ) );  // equation given in paper..
	
	if(edge_stay < SPECT_SPARSE_PROB ){ // if so then delete the edge...
	edge_pairs[i].x = edge_pairs[i].y;
	edge_pairs[i].y = edge_pairs[i].y;
	edge_removed_cnt++;
	} // end-of inner if
 ...
 auto stop1 = high_resolution_clock::now();
    ...
    std::cout << "\nTime taken by edge removal: " << duration1.count() << " microseconds" << std::endl;
    std::cout << "\nTotal number of Edges Removed: " << edge_removed_cnt; 
