	...
	// Get starting timepoint
	  ...
	  while(isleft0deg)
	  {
		...
		std::vector <int> out_degree(n + 1, 0);
	        for(int i=0;i<edge_pairs.size();i++)
		{
		        out_degree[edge_pairs[i][0]]++;		
		        //in_degree[edge_pairs[i][1]]++;		
		}

	        for(int i=0;i<edge_pairs.size();i++)
		{
	                if( ( out_degree[edge_pairs[i][0]] > 0 && out_degree[edge_pairs[i][1]] > 0 ) || ( edge_pairs[i][0] == n || edge_pairs[i][1] == n ) || ( edge_pairs[i][0] == (n-1) || edge_pairs[i][1] == (n-1) ) || ( edge_pairs[i][0] == 1 || edge_pairs[i][1] == 1 ) )
			{
			vector <int> arr(3);
			...
			if( out_degree[edge_pairs[i][0]] <= 0 && out_degree[edge_pairs[i][1]] <= 0  )
			{
			...
			else if( out_degree[edge_pairs[i][0]] <= 0 )
			{
				empty_vertices.push_back( edge_pairs[i][0] );
				total_removed++;
			}
			
			else if( out_degree[edge_pairs[i][1]] <= 0 )
			{
				empty_vertices.push_back( edge_pairs[i][1] );
				total_removed++;
			}	
