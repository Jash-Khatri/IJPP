...
{	
	...
	for(int i=0;i<m;i++)
	{
		...
		fscanf(filePointer, "%d", &v[2] );	// comment for the unweighted graph
	...
	*/
	...
	// Get starting timepoint
	  ...
	  while(isleft0deg)
	  {
		...
		//std::vector <int> out-degree(n + 1, 0);
	        for(int i=0;i<edge_pairs.size();i++)
		{
		        //out-degree[edge_pairs[i][0]]++;		
		        in_degree[edge_pairs[i][1]]++;		
		}

	        for(int i=0;i<edge_pairs.size();i++)
		{
	                if( ( in_degree[edge_pairs[i][0]] > 0 && in_degree[edge_pairs[i][1]] > 0 ) || ( edge_pairs[i][0] == n || edge_pairs[i][1] == n ) || ( edge_pairs[i][0] == (n-1) || edge_pairs[i][1] == (n-1) ) || ( edge_pairs[i][0] == 1 || edge_pairs[i][1] == 1 ) )
			{
			vector <int> arr(3);
	                arr[0] = edge_pairs[i][0];
	                arr[1] = edge_pairs[i][1];
			arr[2] = edge_pairs[i][2];		// comment for the unweighted graph
			//arr[2] = 1;				// comment for the weighted graph			
			...
			if( in_degree[edge_pairs[i][0]] <= 0 && in_degree[edge_pairs[i][1]] <= 0  )
			{
			...
			else if( in_degree[edge_pairs[i][0]] <= 0 )
			{
				empty_vertices.push_back( edge_pairs[i][0] );
				total_removed++;
			}
			
			else if( in_degree[edge_pairs[i][1]] <= 0 )
			{
				empty_vertices.push_back( edge_pairs[i][1] );
				total_removed++;
			}	
